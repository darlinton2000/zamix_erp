    ...
    <div class="box-body">
        <form action="{{ route('users.update', ['id' => $user->id]) }}) }}" enctype="multipart/form-data" method="post">
            ...
            <div class="form-group">
                @if ($user->image)
                    <img class="img-circle" src="{{ url("storage/$user->image")}}" alt="{{ $user->name }}" width="80" height="80">
                @else
                    <img class="img-circle" src="{{ url("/user_default.png")}}" alt="{{ $user->name }}" width="80" height="80">
                @endif
            </div>
            <div class="form-group row">
                <label class="col-sm-2 col-form-label">Nome Completo</label>
                <div class="col-sm-10">
                    <input type="text" name="name" value="{{ $user->name }}" class="form-control" placeholder="Nome Completo" required>
                </div>
            </div>
            <div class="form-group row">
